...
|---|---:|---|
| `official_oracle_fixture_gap` | 50 | Report-only in score output; baseline comparison requires review if this count grows. File upstream reconciliation issues by bundle. |
...
Use bundles rather than 54 one-off issues. The goal is to ask upstream to
...
|---|---:|---|---|
| Paired PDVAL bound oracle review | 4 | Confirm whether `CORE-000698` expected rows should use PDVALMIN and `CORE-000704` should use PDVALMAX. Their positive fixtures now match and are no longer deferred. | Draft 1 in `open-rules-upstream-issue-drafts.md`; candidate table in `open-rules-official-fixture-gap-candidates.md`. |
...
| Positive fixtures with issues | 7 | Confirm whether positive fixtures should be clean or whether issue-bearing positive fixtures need explanatory metadata. The `CORE-000698` and `CORE-000704` positive fixtures now match. | Draft 3 in `open-rules-upstream-issue-drafts.md`. |
...
| Remaining official fixture/oracle gaps | 37 | File smaller follow-ups by semantic family after the first four bundles are reviewed. This includes `CORE-000884`, whose official results contain two context variables that are not declared by the rule's output variables. | Bundle table in `open-rules-official-fixture-gap-candidates.md`. |
...
Do not fail on the accepted 54 `deferred_oracle_gap_skipped` cases, but do not
