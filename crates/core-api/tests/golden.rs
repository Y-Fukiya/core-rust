...
#[test]
fn golden_validates_clinical_fd_sdtm_fixture() {
    let fixtures = fixture_root();
    let outcome = run_validation(ValidateRequest {
        rule_paths: vec![fixtures.join("rules/clinical_fd_sdtm")],
        dataset_paths: vec![fixtures.join("datasets/clinical_fd_sdtm/study_package.json")],
        include_rules: Vec::new(),
        exclude_rules: Vec::new(),
        output_dir: None,
        ..Default::default()
    })
    .expect("run clinical_fd SDTM fixture validation");
    ...
    let result = |rule_id: &str| {
        outcome
            ...
            .expect("clinical_fd fixture result")
    };

    assert_eq!(
        result("CORE-CLINICAL-FD-SDTM-0001").execution_status,
        ExecutionStatus::Passed
    );
    assert_eq!(result("CORE-CLINICAL-FD-SDTM-0001").error_count, 0);

    assert_eq!(
        result("CORE-CLINICAL-FD-SDTM-0002").execution_status,
        ExecutionStatus::Passed
    );
    assert_eq!(result("CORE-CLINICAL-FD-SDTM-0002").error_count, 0);

    assert_eq!(
        result("CORE-CLINICAL-FD-SDTM-0003").execution_status,
        ExecutionStatus::Failed
    );
    assert_eq!(result("CORE-CLINICAL-FD-SDTM-0003").error_count, 4);

    assert_eq!(
        result("CORE-CLINICAL-FD-SDTM-0004").execution_status,
        ExecutionStatus::Failed
    );
    assert_eq!(result("CORE-CLINICAL-FD-SDTM-0004").error_count, 34);
}
